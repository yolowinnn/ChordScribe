...
- **官方歌词**：歌词不再依赖 Gemini 听写（易错），而是联网取网易云**官方歌词**
  (`/api/lyrics`)，作为权威文本喂给 Gemini，只让它做和弦对位 → 歌词准确。
- **调音器**：顶部导航「🎵 调音器」打开，Web Audio 播放吉他/贝斯/尤克里里/小提琴/大提琴
  各弦标准音，按弦校音。系统级工具，独立于曲谱。
- **自动滚动**：看谱时底部悬浮播放条，5 档速度匀速滚动，弹唱不用手翻。
- **和弦图**：谱顶展示全曲和弦的指法图（吉他/尤克里里六线、钢琴键盘高亮）。
